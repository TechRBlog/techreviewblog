...
// ===== X TWEETS ANIMATION =====
function initFloatingTweets() {
    const container = document.getElementById('tweetsContainer');
    if (!container) return;

    // Tech company and influencer tweet data
    const tweetData = [
        // Tech Companies
        {
            name: "TechCrunch",
            handle: "@TechCrunch",
            avatar: "TC",
            content: "Breaking: AI revolution continues to reshape how we work and create. The future is happening now! 🚀",
            time: "2m"
        },
        {
            name: "Wired",
            handle: "@WIRED",
            avatar: "WD",
            content: "WIRED is where tomorrow is realized. Technology isn't just changing the world—it's creating entirely new ones.",
            time: "5m"
        },
        {
            name: "Engadget",
            handle: "@Engadget",
            avatar: "EN",
            content: "The definitive guide to this connected life. Every gadget, every breakthrough, every moment that matters.",
            time: "8m"
        },
        {
            name: "Ars Technica",
            handle: "@arstechnica",
            avatar: "AT",
            content: "Deep analysis of tech trends and expert advice on the fundamental aspects of technology shaping our world.",
            time: "12m"
        },
        {
            name: "The Next Web",
            handle: "@TheNextWeb",
            avatar: "NW",
            content: "International news on technology, business and culture. Following the channels that matter most.",
            time: "15m"
        },
        {
            name: "ZDNet",
            handle: "@ZDNet",
            avatar: "ZD",
            content: "Where technology meets business. Breaking down complex tech for real-world business impact.",
            time: "18m"
        },
        {
            name: "TechRepublic",
            handle: "@TechRepublic",
            avatar: "TR",
            content: "Empowering the People of Business and Technology. Innovation that drives productivity and growth.",
            time: "22m"
        },
        {
            name: "Gigaom",
            handle: "@Gigaom",
            avatar: "GO",
            content: "The leading global voice on emerging technologies. Humanizing tech and providing deep insight on disruptive trends.",
            time: "25m"
        },
        {
            name: "Re/code",
            handle: "@recode",
            avatar: "RC",
            content: "Independent tech news, reviews and events. Bringing you the stories that matter in technology.",
            time: "28m"
        },
        {
            name: "Smart Planet",
            handle: "@SmartPlanet",
            avatar: "SP",
            content: "Innovation that impacts your world. Technology solutions for a sustainable future.",
            time: "30m"
        },
        ...
        },
        {
            ...
            handle: "@codinghorror",
            avatar: "JA",
            content: "Indoor enthusiast. Building better software communities one line of code at a time.",
            time: "2h"
        },
        {
            name: "Jason Fried",
            handle: "@jasonfried",
            avatar: "JF",
            content: "Building better work experiences. Remote work isn't the future—it's the present.",
            time: "3h"
        },
        {
            name: "John Resig",
            handle: "@jeresig",
            avatar: "JR",
            content: "JavaScript continues to evolve and shape the web. Exciting times ahead for developers! 💻",
            time: "4h"
        },
        {
            name: "Kent Beck",
            handle: "@KentBeck",
            avatar: "KB",
            content: "Programmer, author, father, husband. Making software development more human, one test at a time.",
            time: "5h"
        },
        {
            name: "Martin Fowler",
            handle: "@martinfowler",
            avatar: "MF",
            content: "Good software design is about making complex systems simple to understand and modify.",
            time: "6h"
        },
        {
            name: "Ward Cunningham",
            handle: "@WardCunningham",
            avatar: "WC",
            content: "Objects, Patterns, Agile, Wiki. The foundations of modern software development continue to evolve.",
            time: "7h"
        },
        {
            ...
            handle: "@defunkt",
            avatar: "CW",
            content: "Before cofounding GitHub I applied for an engineering job at Yahoo and didn't get it. Don't let others discourage you.",
            time: "8h"
        },
        {
            name: "Scott Belsky",
            handle: "@scottbelsky",
            avatar: "SB",
            content: "Leading the creative world through technology. Making ideas happen, one innovation at a time.",
            time: "9h"
        },
        {
            name: "Leo Laporte",
            handle: "@LeoLaporte",
            avatar: "LL",
            content: "Podcaster, broadcaster and tech pundit. Bringing technology to the people since the dawn of the internet.",
            time: "10h"
        },
        {
            name: "Neil Patel",
            handle: "@neilpatel",
            avatar: "NP",
            content: "Entrepreneur, investor, advisor. Building SaaS analytics companies that change how we understand data.",
            time: "11h"
        },
        {
            name: "Alexia Tsotsis",
            handle: "@alexia",
            avatar: "AT",
            content: "Breaking news and hearts in the tech world. The stories behind the startups that change everything.",
            time: "12h"
        }
    ];
    ...
    // Animation settings
    let tweetCount = 0;
    const maxTweets = 1; // Only one tweet visible at a time to prevent overlapping
    const tweetInterval = 6000; // Time between new tweets (6 seconds for better reading)
    const tweetDisplayTime = 8000; // How long each tweet stays visible (8 seconds)

    function createTweet() {
        // Don't create new tweets if max is reached
        const currentTweets = container.querySelectorAll('.x-tweet');
        if (currentTweets.length >= maxTweets) return;

        // Get random tweet data
        const tweet = tweetData[Math.floor(Math.random() * tweetData.length)];

        // Create tweet element
        const tweetElement = document.createElement('div');
        tweetElement.className = 'x-tweet';

        // Use simple fade-in animation for dedicated container
        tweetElement.style.animation = 'fadeInUp 0.5s ease-out forwards';
        ...
        // Add click handler to open X
        tweetElement.addEventListener('click', () => {
            const xUrl = `https://x.com/${tweet.handle.substring(1)}`;
        ...
        // Remove tweet after display time
        setTimeout(() => {
            if (tweetElement.parentNode) {
                tweetElement.style.animation = 'fadeOut 0.5s ease-out forwards';
                setTimeout(() => {
                    if (tweetElement.parentNode) {
                        tweetElement.remove();
                    }
    ...
    // Start the tweet animation cycle
    function startTweetCycle() {
        createTweet();

        // Create new tweets at intervals
        setInterval(() => {
            createTweet();
        }, tweetInterval);
    }

    // Only start if we're on a page with the hero section
    const heroSection = document.querySelector('.hero');
    if (heroSection) {
        // Start after a short delay
        setTimeout(startTweetCycle, 3000);
    }
}
...
// Modified initialization to respect accessibility
function initFloatingTweetsAccessible() {
    if (respectsReducedMotion()) {
        console.log('Floating tweets disabled due to reduced motion preference');
        return;
    }
    initFloatingTweets();
}